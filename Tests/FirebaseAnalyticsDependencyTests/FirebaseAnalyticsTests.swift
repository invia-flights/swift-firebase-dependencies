...
final class FirebaseAnalyticsTests: XCTestCase {
	func testItLogsScreenViews() async throws {
		...
		try await sut.log(.screenView(.init(screenClass: "ViewController", screenName: "SearchResults")))
	...
	func testItLogsViewPromotionEvents() async throws {
		...
		let sut: FirebaseAnalyticsClient = .live(analytics: analytics)
		try await sut.log(.viewPromotion(.init(creativeName: "Summer Sale", creativeSlot: "Banner", items: nil, locationID: nil, promotionID: nil, promotionName: nil)))
	...
	func testItLogsSelectPromotionEvents() async throws {
		...
		let sut: FirebaseAnalyticsClient = .live(analytics: analytics)
		try await sut.log(.selectPromotion(.init(creativeName: "Summer Sale", creativeSlot: "Banner", items: nil, locationID: nil, promotionID: nil, promotionName: nil)))
	...
	func testItLogsAddToCartEvents() async throws {
		...
		let sut: FirebaseAnalyticsClient = .live(analytics: analytics)
		try await sut.log(.addToCart(.init(value: .init(amount: 123, currency: "EUR"), items: [
			.init(id: "123")
		])))
	...
	func testItLogsAddToWishlistEvents() async throws {
		...
		let sut: FirebaseAnalyticsClient = .live(analytics: analytics)
		try await sut.log(.addToWishList(.init(value: .init(amount: 123, currency: "EUR"), items: [
			.init(id: "123")
		])))
